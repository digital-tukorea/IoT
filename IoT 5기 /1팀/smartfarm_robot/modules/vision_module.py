...
# API의 language/details 파라미터로 한국어 응답을 우선 시도하지만(diagnose_disease_type
# 참고), 병명에 따라 한국어 데이터 자체가 없어 영문 원본이 그대로 오는 경우가
# 실측으로 확인되어(예: "Botrytis" -> local_name도 "Botrytis") 이 사전을
# 확실한 안전장치(fallback)로 둔다.
...
class DetectionResult:
    ...
    disease_type: Optional[str] = None           # Kindwise API가 준 한국어 병명
...
class VisionModule:
    ...
    # ★★ crop.health에서 plant.id로 전환함 (실측 근거):
    #   crop.health는 딸기 사진에서 작물 인식 자체를 실패함(apple 0.16%) - 사실상 딸기 미지원.
    #   plant.id는 같은 사진에서 학명(Fragaria × ananassa) 97.7%로 정확히 인식했고,
    #   병명(Botrytis, 93.9%)도 crop.health가 찾은 grey mold(Botrytis cinerea)와 일치함.
    #   plant.id가 3만 종 이상을 커버하는 훨씬 넓은 데이터베이스라 이쪽을 기본으로 채택.
    def diagnose_disease_type(self, box_bgr):
        ...
        try:
            ...
            # ★ 1차: API가 준 한국어 통용명(local_name) 시도
            raw_details = getattr(top, "details", None)
            local_name_ko = raw_details.get("local_name") if raw_details else None

            # ★ API가 해당 언어로 번역을 못 해서 영문 원본(name)을 그대로
            # local_name에 돌려주는 경우가 있다 (실측 확인됨). 이 경우
            # "번역 안 됨"으로 간주하고 우리 사전으로 넘어가야 한다.
            if local_name_ko and local_name_ko.strip().lower() == top.name.strip().lower():
                local_name_ko = None

            # ★ 2차: null이거나 번역 안 된 경우 우리 사전으로 대체
            disease_name_ko = local_name_ko or translate_disease_name(top.name)
    ...
    def detect(self, frame, zone_id):
        ...
        results = self.model(
            ...
            imgsz=self.imgsz,
            # ★ agnostic_nms: 기본 NMS(중복 제거)는 "같은 클래스끼리"만 겹치는
            # 박스를 정리한다. 그래서 같은 물체가 서로 다른 클래스로 겹쳐서
            # 판단되면(예: 딸기 하나가 grape로도, strawberry로도 잡히는 경우)
            # 둘 다 살아남는 문제가 있었다. agnostic_nms=True로 하면 클래스
            # 구분 없이 겹치는 박스들 중 신뢰도가 가장 높은 것 하나만 남긴다.
            agnostic_nms=self.config.get("agnostic_nms", True),
            verbose=False,
        )
