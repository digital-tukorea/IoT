...
⭐ 이 파일만 5개 모듈(vision/motion/mqtt/upload/env_sensor)의 존재를 전부
   알고 있다. 모듈끼리는 서로를 import하지 않는다 — 항상 이 컨트롤러를
   거쳐서만 연결된다.
...
class RobotController:
    def __init__(self, config):
        ...
        print("[초기화] 온습도 센서 모듈 준비 중 (라즈베리파이 GPIO 직결)...")
    ...
    # ── 분석이 끝난 결과들을 처리 (탐지가 있으면 dispatch 큐로 넘김) ──
    def _process_ready_results(self):
        while True:
            ...
            # ★ 탐지된 게 하나도 없으면 로그에 명시적으로 남긴다 (기존에는
            # 이 경우 아무 로그도 안 찍혀서 "분석했는데 없었다"인지 "분석 자체가
            # 안 됐다"인지 구분이 안 되는 문제가 있었다).
            if not detections:
            ...
            for detection in detections:
                ...
                # ★ 업로드/발행은 여기서 바로 하지 않고 큐에 넣기만 함
                # (별도 스레드가 처리 -> 네트워크 지연이 메인 루프를 안 막음)
                self._dispatch_queue.put((detection, zone_id))

    # ── ★ HTTP 업로드/MQTT 발행을 메인 루프와 분리된 스레드에서 처리 ──
    def _dispatch_worker(self):
        while True:
            item = self._dispatch_queue.get()
            if item is None:   # 종료 신호
                break
            detection, zone_id = item
    ...
    def _dispatch_detection(self, detection, zone_id):
        ...
        # (원본 전체 프레임은 save_raw_captures가 이미 저장하고 있고, 이건
        # 그중 "실제로 인식된 부분만" 따로 잘라서 보관하는 용도)
    ...
    def _publish_env_log(self):
        ...
        env_payload = {
            ...
            "humidity": round(humidity_percent, 1) if humidity_percent is not None else None,
        }
    ...
    def run(self):
        ...
        camera_indices = self.cfg.get("camera_indices", [0])
        camera_labels = self.cfg.get("camera_labels", {})
        caps = []
        ...
        # ★ [캡처 트리거] 더 이상 타이머로 "N초마다 찍어라"가 아니라,
        # 아두이노가 마커에서 자연스럽게 멈추는 순간(STATUS="PAUSED")을
        # 감지해서 캡처 트리거로 재사용한다.
        ...
        try:
            while True:
                ...
                # 위치(트리거 지점)"로만 쓰인다.
                zone_id = self.cfg.get("fixed_zone_id", "a1")
                ...
                if self.cfg.get("show_preview", True):
                    ...
                    for cam_idx, frame in preview_frames.items():
                        label = camera_labels.get(cam_idx, str(cam_idx))
                        disp = frame.copy()
                        cv2.putText(disp, f"{label} | {status_text} | Zone: {zone_id}",
                                    (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7,
                                    (0, 255, 0) if self.is_ai_running else (0, 165, 255), 2)
                        cv2.imshow(f"Smart Farm AI Camera - {label}", disp)
                ...
                if self.is_ai_running and just_entered_paused:
                    ...
                    for cam_idx, cap in zip(camera_indices, caps):
                        ...
                        # ★ YOLO가 아무것도 못 찾더라도(0건 탐지), 원본 촬영본은
                        # 항상 로컬에 남긴다. 왜 못 찾았는지 나중에 눈으로
                        # 확인하거나, 데이터셋 보강용으로 재활용할 때 유용하다.
                        if self.cfg.get("save_raw_captures", True):
                            cam_label = camera_labels.get(cam_idx, str(cam_idx))
                            capture_filename = f"{now_str}_{zone_id}_{cam_label}.jpg"
                            capture_path = os.path.join(self.cfg["capture_dir"], capture_filename)
                            success_save = cv2.imwrite(capture_path, frame)
                            if not success_save:
                                print(f"  ❌ 원본 캡처 저장 실패: {capture_path}")
    ...
    def _shutdown(self):
        ...
        # ★ 업로드/발행 스레드도 큐에 남은 것 다 처리하고 안전하게 종료
