...
from smart_desk.config.settings import AutomationSettings, Settings, VisionSettings
...
def test_vacant_park_delay_defaults_to_five_seconds_and_reads_environment(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    assert AutomationSettings().park_delay_seconds == 5.0

    monkeypatch.setenv("SMART_DESK_AUTOMATION__PARK_DELAY_SECONDS", "7.5")

    assert Settings(_env_file=None).automation.park_delay_seconds == 7.5


@pytest.mark.parametrize("value", [-1, float("inf"), float("nan")])
def test_invalid_vacant_park_delay_is_rejected(value: float) -> None:
    with pytest.raises(ValidationError):
        AutomationSettings(park_delay_seconds=value)


def test_vision_uncertainty_grace_defaults_to_five_seconds_and_reads_environment(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    assert AutomationSettings().vision_uncertainty_grace_seconds == 5.0

    monkeypatch.setenv("SMART_DESK_AUTOMATION__VISION_UNCERTAINTY_GRACE_SECONDS", "3.5")

    assert Settings(_env_file=None).automation.vision_uncertainty_grace_seconds == 3.5


@pytest.mark.parametrize("value", [-1, 31, float("inf"), float("nan")])
def test_invalid_vision_uncertainty_grace_is_rejected(value: float) -> None:
    with pytest.raises(ValidationError):
        AutomationSettings(vision_uncertainty_grace_seconds=value)
...
def test_voice_is_disabled_by_default_without_api_key() -> None:
    ...
    assert settings.voice.wakeword_inference_interval_frames == 25
    assert settings.voice.realtime_connect_timeout_seconds == 10.0
