...
def test_safe_realtime_provider_fields_reach_the_log_line() -> None:
    payload = json.loads(JsonFormatter().format(record(
        component="assistant.realtime", event="episode_failed",
        exception_type="RealtimeProviderError",
        provider_error_type="invalid_request_error",
        provider_error_code="invalid_value",
        provider_error_param="session.audio.input.format",
    )))
    assert payload["exception_type"] == "RealtimeProviderError"
    assert payload["provider_error_type"] == "invalid_request_error"
    assert payload["provider_error_code"] == "invalid_value"
    assert payload["provider_error_param"] == "session.audio.input.format"
