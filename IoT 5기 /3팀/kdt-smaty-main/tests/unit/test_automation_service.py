...
def multiple_vision(
    pair: tuple[float, float], *, usable: bool = False
) -> VisionSnapshot:
    snapshot = vision(pair)
    upper = CameraObservation(True, pair[0], pair[0], NOW, None, 2)
    lower = CameraObservation(True, pair[1], pair[1], NOW, None, 2, snapshot.stable_posture)
    return VisionSnapshot(
        upper,
        lower,
        PresenceStatus.MULTIPLE,
        PresenceStatus.MULTIPLE,
        snapshot.raw_posture,
        snapshot.stable_posture,
        NOW,
        NOW,
        usable,
...
async def test_multiple_people_with_stable_posture_can_select_height() -> None:
    clock, desk, users = Clock(), FakeDesk(), FakeUsers(user())
    camera = FakeVision(multiple_vision((1, 1)))
    service = service_for(users=users, camera=camera, desk=desk, clock=clock)

    await service._observe_once()
    camera.snapshot = multiple_vision((2, 2))
    await service._observe_once()
    clock.advance(1)
    camera.snapshot = multiple_vision((3, 3))
    await service._observe_once()

    snapshot = service.get_snapshot()
    assert snapshot.target_height_cm == 75
    assert snapshot.blocked_reason_codes == ("AUTOMATIC_EXECUTION_DISABLED",)


async def test_short_vision_gap_pauses_but_preserves_posture_candidate() -> None:
    clock, desk, users = Clock(), FakeDesk(), FakeUsers(user())
    camera = FakeVision(vision((1, 1)))
    service = service_for(users=users, camera=camera, desk=desk, clock=clock)
    await observe(service, camera, (1, 1), clock)
    await observe(service, camera, (2, 2), clock)
    original_due = service.get_snapshot().initial_move_due_at
    ...
    await observe(
        ...
        .4,
        usable=False,
        reasons=(BlockCode.LOWER_FRAME_STALE,),
    )
    during_gap = service.get_snapshot()
    assert during_gap.posture_candidate is PostureStatus.SITTING
    assert during_gap.last_transition_reason == "VISION_GRACE"

    await observe(service, camera, (4, 4), clock, 2)
    recovered = service.get_snapshot()
    assert recovered.posture_candidate is PostureStatus.SITTING
    assert recovered.initial_move_due_at == original_due + timedelta(seconds=2)
    assert recovered.target_height_cm is None

    await observe(service, camera, (5, 5), clock, .6)
    assert service.get_snapshot().target_height_cm == 75


async def test_long_vision_gap_discards_candidate_and_restarts_from_fresh_baseline() -> None:
    clock, desk, users = Clock(), FakeDesk(), FakeUsers(user())
    camera = FakeVision(vision((1, 1)))
    service = service_for(users=users, camera=camera, desk=desk, clock=clock)
    await observe(service, camera, (1, 1), clock)
    ...
    await observe(
        ...
        .2,
        usable=False,
        reasons=(BlockCode.LOWER_CAMERA_UNAVAILABLE,),
    )

    await observe(service, camera, (4, 4), clock, 5.1)

    snapshot = service.get_snapshot()
    assert snapshot.posture_candidate is None
    assert snapshot.target_height_cm is None
    assert snapshot.last_transition_reason == "VISION_RECOVERED_AFTER_GRACE"


async def test_stable_vacancy_immediately_discards_posture_candidate() -> None:
    clock, desk, users = Clock(), FakeDesk(), FakeUsers(user())
    camera = FakeVision(vision((1, 1)))
    service = service_for(users=users, camera=camera, desk=desk, clock=clock)
    await observe(service, camera, (1, 1), clock)
    ...
    await observe(service, camera, (3, 3), clock, .1, vacant=True)

    snapshot = service.get_snapshot()
    assert snapshot.posture_candidate is None
    assert snapshot.initial_move_due_at is None
    assert snapshot.blocked_reason_codes == ("PRESENCE_NOT_SINGLE",)
...
async def test_sessionless_stop_failure_latch_blocks_fresh_vacant_park() -> None:
    ...
    await observe(service, camera, (3, 3), clock, 5, vacant=True)
...
async def test_five_second_park_resets_tilt_and_presence_cancels_both_motions() -> None:
    clock, desk, users, led = Clock(), FakeDesk(), FakeUsers(user(registered=True)), FakeWled()
    camera = FakeVision(vision((1, 1)))
    service = service_for(
        users=users,
        camera=camera,
    ...
    await observe(service, camera, (3, 3), clock, 4.9, vacant=True)
    await flush_background_tasks()
    assert not [call for call in desk.calls if call[0] == "target"]
    assert tilt.levels == []

    await observe(service, camera, (4, 4), clock, 0.1, vacant=True)
    await flush_background_tasks()
    assert ("target", 75.0) in desk.calls
    assert tilt.levels == [0]

    await observe(service, camera, (5, 5), clock)
    assert [call for call in desk.calls if call[0] == "stop"]
    assert tilt.stops == ["PARK 취소 틸팅 정지"]


async def test_park_tilt_failure_does_not_rollback_the_safe_height_target() -> None:
    clock, desk, users = Clock(), FakeDesk(), FakeUsers(None)
    camera = FakeVision(vision((1, 1), vacant=True))
    service = service_for(users=users, camera=camera, desk=desk, clock=clock, execute=True)
    tilt = FakeTilt(error=RuntimeError("tilt unavailable"))
    service.set_tilt(tilt)

    await observe(service, camera, (1, 1), clock, vacant=True)
    await observe(service, camera, (2, 2), clock, 5, vacant=True)
    await flush_background_tasks()
...
async def test_disabled_automatic_execution_does_not_reset_park_tilt() -> None:
    clock, desk, users = Clock(), FakeDesk(), FakeUsers(None)
    camera = FakeVision(vision((1, 1), vacant=True))
    service = service_for(users=users, camera=camera, desk=desk, clock=clock)
    tilt = FakeTilt()
    ...
    assert tilt.levels == []
    assert service.get_snapshot().blocked_reason_codes == ("AUTOMATIC_EXECUTION_DISABLED",)
