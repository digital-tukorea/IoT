...
async def test_realtime_runtime_reports_connect_timeout() -> None:
    async def connect_forever() -> Transport:
        await asyncio.Event().wait()
        raise AssertionError("unreachable")
    ...
    runtime = RealtimeVoiceRuntime(
        connect_forever,
        handler,
        config=RealtimeVoiceConfig(connect_timeout_seconds=0.01),
    )
    events = [event async for event in runtime.run_audio(chunks(b"\x02\x00"))]

    assert [(event.type, event.error_code) for event in events] == [
        (VoiceRuntimeEventType.ERROR, "voice_provider_connect_timeout")
    ]
...
async def test_realtime_runtime_maps_exhausted_provider_credit() -> None:
    transport = Transport([{
        "type": "error",
        "error": {
    ...
    assert [(event.type, event.error_code) for event in events] == [
        (VoiceRuntimeEventType.ERROR, "voice_provider_quota_exhausted")
    ]
