...
def build(
    tmp_path: Path,
    *,
    homing_completion_timeout_seconds: float = 30.0,
    ...
    return (
        TiltController(
            ...
            settings,
            TaskManager(),
            homing_completion_timeout_seconds=homing_completion_timeout_seconds,
        ),
        mqtt,
        link,
...
async def test_homing_timeout_stops_and_recovers_level_zero(tmp_path: Path) -> None:
    controller, _mqtt, link = build(tmp_path, homing_completion_timeout_seconds=0.01)
    await controller.start()
    try:
        await ready(controller, link)
        await controller.set_target(0)
        await wait_until(lambda: any(command.startswith("RUN DOWN") for command in link.written))
        await wait_until(lambda: "SET_POSITION 0.00" in link.written)

        run_index = next(i for i, command in enumerate(link.written) if command.startswith("RUN DOWN"))
        stop_index = link.written.index("STOP", run_index)
        position_index = link.written.index("SET_POSITION 0.00", stop_index)
        assert run_index < stop_index < position_index
        snapshot = controller.get_snapshot()
        assert snapshot.state is TiltState.AT_TARGET
...
async def test_homing_completion_cancels_timeout_recovery(tmp_path: Path) -> None:
    controller, _mqtt, link = build(tmp_path, homing_completion_timeout_seconds=0.05)
    await controller.start()
    try:
        await ready(controller, link)
        await controller.set_target(0)
        await wait_until(lambda: any(command.startswith("RUN DOWN") for command in link.written))
        link.push({"event": "stopped", "reason": "manual_complete", "position_valid": False})
        await wait_until(lambda: controller.get_snapshot().state is TiltState.AT_TARGET)
        await asyncio.sleep(0.06)

        assert link.written.count("STOP") == 0
        assert link.written.count("SET_POSITION 0.00") == 1
        assert controller.get_snapshot().position_valid is True
    finally:
        await controller.stop()


async def test_manual_stop_cancels_homing_timeout_recovery(tmp_path: Path) -> None:
    controller, _mqtt, link = build(tmp_path, homing_completion_timeout_seconds=0.03)
    await controller.start()
    try:
        await ready(controller, link)
        await controller.set_target(0)
        await wait_until(lambda: any(command.startswith("RUN DOWN") for command in link.written))
        await controller.stop_motion("사용자가 영점 이동을 정지했습니다.")
        await asyncio.sleep(0.04)

        assert "SET_POSITION 0.00" not in link.written
        snapshot = controller.get_snapshot()
        assert snapshot.state is TiltState.STOPPED
        assert snapshot.position_valid is False
    finally:
        await controller.stop()


async def test_homing_timeout_does_not_set_position_when_stop_publish_fails(tmp_path: Path) -> None:
    controller, _mqtt, link = build(tmp_path, homing_completion_timeout_seconds=0.01)
    await controller.start()
    try:
        await ready(controller, link)
        await controller.set_target(0)
        await wait_until(lambda: any(command.startswith("RUN DOWN") for command in link.written))
        link.fail_prefix = "STOP"
        await wait_until(lambda: controller.get_snapshot().state is TiltState.ERROR)

        assert "STOP" in link.written
        assert "SET_POSITION 0.00" not in link.written
        assert "정지 명령을 전송하지 못했습니다" in (controller.get_snapshot().last_error or "")
    finally:
        link.fail_prefix = None
        await controller.stop()


async def test_noncompletion_stop_still_recovers_when_homing_timeout_expires(tmp_path: Path) -> None:
    controller, _mqtt, link = build(tmp_path, homing_completion_timeout_seconds=0.02)
    await controller.start()
    try:
        await ready(controller, link)
        await controller.set_target(0)
        await wait_until(lambda: any(command.startswith("RUN DOWN") for command in link.written))
        link.push({"event": "stopped", "reason": "unexpected_stop", "position_valid": False})
        await wait_until(lambda: controller.get_snapshot().state is TiltState.ERROR)
        await wait_until(lambda: "SET_POSITION 0.00" in link.written)
