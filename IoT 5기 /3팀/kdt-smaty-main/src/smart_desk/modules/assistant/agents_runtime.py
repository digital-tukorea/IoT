...
def build_smart_desk_agent(
    ...
    *,
    ...
    """Assemble the single Smart Desk Agent shared by voice and offline evaluation.

    음성 pipeline 밖에서도 같은 instruction·model·tool 조합을 재현할 수 있어야
    intent 평가가 운영 동작을 측정한다. optional dependency는 여기서만 import한다.
    """
    from agents import Agent, ModelSettings, WebSearchTool
    from agents.models.openai_responses import OpenAIResponsesModel

    hosted = [WebSearchTool()] if include_web_search else []
    return Agent(
        name="Smart Desk",
...
class AgentsVoiceRuntime:
    ...
    def build(
        ...
        *,
        ...
        agent = build_smart_desk_agent(client, config=config, tools=tools)
