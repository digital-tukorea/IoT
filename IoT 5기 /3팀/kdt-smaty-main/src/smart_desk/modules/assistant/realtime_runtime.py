...
class RealtimeProviderError(RuntimeError):
    ...
    def public_error_code(self) -> str:
        """Map stable provider identifiers without exposing its message body."""
        if (
            self.provider_type == "insufficient_quota"
            or self.provider_code in {"credit_balance_exhausted", "insufficient_quota"}
        ):
            return "voice_provider_quota_exhausted"
        if (
            self.provider_type == "rate_limit_error"
            or self.provider_code == "rate_limit_exceeded"
        ):
...
class RealtimeConnectTimeout(RuntimeError):
    """Realtime WebSocket이 운영 연결 제한 안에 열리지 않은 경우다."""
    ...
    async def run_audio(self, chunks: AsyncIterable[bytes]) -> AsyncIterator[VoiceRuntimeEvent]:
        ...
        try:
            ...
            try:
                ...
                raise RealtimeConnectTimeout("realtime_connect_timeout") from error
            ...
            error_code = (
                ...
                else "voice_provider_connect_timeout"
                if isinstance(error, RealtimeConnectTimeout)
                else "voice_response_not_completed"
                if isinstance(error, RealtimeResponseStatusError)
                else error.public_error_code
                if isinstance(error, RealtimeProviderError)
                else error.code
