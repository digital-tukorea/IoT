...
TILT_HOMING_WATCHDOG_TASK_NAME = "tilt-homing-watchdog"
...
# ESP32가 RUN 완료 이벤트를 잃어도 수동 제어가 영구 잠기지 않게 하는 서버측 상한이다.
HOMING_COMPLETION_TIMEOUT_SECONDS = 30.0
...
class TiltController:
    ...
    def __init__(
        ...
        *,
        homing_completion_timeout_seconds: float = HOMING_COMPLETION_TIMEOUT_SECONDS,
    ) -> None:
        if homing_completion_timeout_seconds <= 0:
            raise ValueError("영점 복구 제한 시간은 0보다 커야 합니다.")
        self._link = link
        self._levels = levels
        self._mqtt = mqtt
        ...
        self._homing_watchdog_task: asyncio.Task[None] | None = None
        self._homing_completion_timeout_seconds = homing_completion_timeout_seconds
    ...
    async def stop(self) -> None:
        ...
        await self._cancel_tasks(
            self._motion_task,
            self._homing_watchdog_task,
            self._prepare_task,
            self._reader_task,
        )
        self._motion_task = None
        self._homing_watchdog_task = None
    ...
    async def _send_home(self, command_generation: int) -> None:
        ...
        await self._publish_status()
        await self._cancel_homing_watchdog()
        self._homing_watchdog_task = self._task_manager.create(
            TILT_HOMING_WATCHDOG_TASK_NAME,
            self._watch_homing_timeout(command_generation),
            critical=False,
        )

    async def _watch_homing_timeout(self, command_generation: int) -> None:
        """ESP 완료 이벤트가 30초 안에 없으면 정지 후 현재 바닥을 영점으로 삼는다."""

        try:
            await asyncio.sleep(self._homing_completion_timeout_seconds)
            async with self._state_lock:
                if (
                    not self._running
                    or command_generation != self._command_generation
                    or self._homing_generation != command_generation
                ):
                    return
                firmware = self._snapshot.firmware

            # 위치를 확정하기 전에 반드시 모터 정지를 먼저 발행한다. 같은 MQTT 연결의
            # QoS 1 메시지 순서에 따라 ESP32는 STOP 뒤 SET_POSITION을 처리한다.
            if not await self._link.write_line("STOP"):
                await self._mark_motion_error(
                    command_generation,
                    "30초 영점 복구 후 틸팅 정지 명령을 전송하지 못했습니다.",
                )
                return
            if not await self._is_current_motion(command_generation):
                return
            LOGGER.warning(
                "틸팅 영점 완료 이벤트가 없어 30초 제한 시간 뒤 0단계로 복구합니다.",
                extra={"component": "tilt", "event": "tilt_homing_timeout_recovered"},
            )
            await self._finish_homing(firmware, cancel_watchdog=False)
        finally:
            if self._homing_watchdog_task is asyncio.current_task():
                self._homing_watchdog_task = None
    ...
    async def _cancel_homing_watchdog(self) -> None:
        task, self._homing_watchdog_task = self._homing_watchdog_task, None
