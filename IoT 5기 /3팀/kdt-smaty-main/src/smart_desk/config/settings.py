...
class AutomationSettings(BaseModel):
    ...
    # 이미 안정 자세 후보를 확인 중일 때 카메라나 pose 결과가 잠깐 끊겨도
    # 지금까지 센 시간을 버리지 않는다. 유예 중 끊긴 시간 자체는 자세 유지
    # 시간으로 세지 않으며, 오래 끊기거나 실제 VACANT이면 후보를 폐기한다.
    vision_uncertainty_grace_seconds: float = Field(
        default=5.0, ge=0, le=30, allow_inf_nan=False
    )
    ...
    # session 종료 뒤에도 새 사람이나 수동 조작이 아닌 지속된 빈자리인지 짧게
    # 재확인한 다음 75cm PARK와 틸트 영점 복귀를 시작한다.
    park_delay_seconds: float = Field(default=5.0, ge=0, le=300, allow_inf_nan=False)
...
class VoiceSettings(BaseModel):
    ...
    # 대기 중 ONNX 추론은 오디오 입력 창마다 한 번만 실행해
    # 냉각 없는 Raspberry Pi의 지속 부하와 열 제한을 피한다.
    wakeword_inference_interval_frames: int = Field(default=25, ge=1, le=25)
    ...
    realtime_connect_timeout_seconds: float = Field(default=10.0, gt=0, le=15)
