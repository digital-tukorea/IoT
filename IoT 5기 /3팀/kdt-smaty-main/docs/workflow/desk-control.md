...
신원 상태(`MATCHED`, `NO_FACE`)와 높이 판단을 분리한다. 상단 재실이 한 명 또는 여러 명이고
하단의 `SITTING`/`STANDING`이 안정화된 뒤 자세 확인 시간을 채우면 목표를 만든다. 자세 확인
도중 카메라·frame·pose가 5초 이내로 잠깐 불확실해지면 기존 후보를 보존하되, 끊긴 시간은
자세 유지 시간으로 세지 않는다. 실제 `VACANT`가 안정화되거나 불확실성이 5초를 넘으면 후보를
폐기한다.
...
Vision의 다중 사용자는 안정 자세가 있으면 AUTO 높이 판단을 허용한다. count·timestamp 불일치,
자세 귀속 불가와 stale 결과는 자세 후보 확인 중 5초 유예를 적용하며, 유예를 넘기면 AUTO와
PARK만 차단한다. 이미 Desk가 접수한 자동 목표는 Vision 흔들림으로 중단하지 않고 DeskController가
높이·relay 안전 조건으로 완료하거나 정지한다. HOLD, 직접 목표와 STOP은 허용한다.
...
`PARK_WAITING`을 시작한다. 두 카메라의 fresh VACANT가 5초 계속되고 활성 수동 의도·새
session이 없으면 75cm PARK 목표와 틸트 0단계 복귀를 함께 요청한다. 이 높이 목표는
AUTO·Dashboard와 동일하게
...
진행 park를 취소하고 PARK 높이·틸트 이동을 STOP한다. 서버 시작 직후 과거 VACANT나 retained
height로 움직이지 않는다.
