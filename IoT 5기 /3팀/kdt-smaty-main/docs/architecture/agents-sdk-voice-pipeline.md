...
- 같은 의도의 다양한 발화가 기대한 tool과 인자로 이어진다.

### intent 파라프레이즈 평가

tool 계약 test는 tool을 직접 호출해 검증하므로, model이 발화를 듣고 **어떤
tool을 고르는지**는 확인하지 못한다. 이 공백은 `tests/eval/`의 파라프레이즈
평가셋이 메운다. 발화만 주고 실제 `Runner.run()`이 고른 tool과 인자를 기대값과
대조한다.

Agent 조립은 `build_smart_desk_agent()` 하나를 음성 경로와 평가가 함께 쓴다.
평가가 운영과 다른 instruction이나 tool 집합을 측정하면 의미가 없기 때문이다.

OpenAI API를 호출하므로 CI에서 상시 실행하지 않고 `scripts/eval_voice_intents.py`
opt-in 스크립트로 분리한다. 실행 방법과 데이터셋 형식은 [tests/eval/README.md](../../tests/eval/README.md)를
따른다. tool 이름·인자 오탈자는 API 없이 도는 `tests/unit/test_eval_datasets.py`가
잡는다.
