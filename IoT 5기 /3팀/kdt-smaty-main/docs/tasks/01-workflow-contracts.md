...
| `vacantParkDelaySeconds` | 5초 | session 종료 후 75cm park·틸트 0단계 복귀 대기 |
...
session 종료 뒤 상단의 fresh한 `VACANT`가 5초 동안 계속되고 새 session·활성 수동
의도가 없으면 75cm `PARK` 목표를 `DeskController.set_target()`에 요청하고 틸트 0단계
복귀를 함께 요청한다. 높이 표시기가
...
| fresh `VACANT` 5초 | 없음 | 없음 | 75cm PARK·틸트 0단계 | 조건 유지 시 시작 | 주차 이동 표시 |
...
- 안정 VACANT 전에 session을 끝내지 않고 session 종료 후 fresh VACANT 5초 전에는 park하지
  않는다.
...
- [x] 얼굴 lifecycle, 서버 재시작과 5초 park·틸트 복귀 순서를 확정했다.
