...
- [x] session 종료 후 fresh VACANT 5초 뒤 75cm PARK와 틸트 0단계 복귀를 만들고 사람
  후보·수동 명령에서 취소한다. 높이
...
- 반대 자세 흔들림과 5초 이내 Vision 공백은 확인 시간을 보존하되 공백 자체는 timer에 더하지
  않고, 안정된 다중 재실도 `SITTING`/`STANDING` 높이 목표를 만들 수 있다.
...
- session 종료 후 fresh VACANT 5초 전에는 park하지 않고 사람 후보·수동 명령·오류가 PARK
  높이·틸트 이동을 STOP한다.
