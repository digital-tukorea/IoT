...
* **해결 방안**:
  1. **1차 후진 (끼임 해제)**: 서보 암이 내려간 상태 그대로 1.5초 후진(`TIME_PUSH_FRONT_RETURN_MS`)하여 옷걸이와의 압착력 해제
  2. **서보 상승**: 무부하 상태에서 안전하게 서보 암 상승 (`SERVO_ANGLE_UP`)
  3. **2차 후진 (정위치 복귀)**: 서보가 올라간 상태에서 1.5초 추가 후진(`TIME_RETURN_TO_TARGET_MS`)하여 타겟 의류 정위치 정렬
...
1. **사용자 호출**: Android 앱에서 날씨/일정 기반 추천 코디 선택 → MQTT (`rail/target_qr`)로 타겟 의류 ID 발행
2. **탐색 주행**: UNO R4가 모터 구동 → 바코드 스캐너(UART)로 옷걸이 태그 실시간 스캔
3. **1차 젖힘 (뒤 옷 밀기)**: 목표 QR 일치 즉시 급제동 → 서보 하강 → 후진 밀기 → 뒤 옷 공간 확보
4. **2차 젖힘 (앞 옷 밀기 & 복귀)**: 전진 주행 → ToF 피크 감지 → 서보 하강 후 전진 밀기 → 2단계 분리 복귀로 타겟 정위치 정렬
5. **조명 및 수거 감시**: WS2812B White LED 점등 → ToF 센서로 실시간 거리 감시(500mm 초과 시 옷 꺼냄 감지)
6. **자동 원점 복귀**: 옷 수거 감지 5초 후 포토 인터럽터(원점 센서)까지 후진 복귀 후 대기 모드 진입
